#!/usr/bin/env bash
# Builds everything a GitHub release carries into dist/release/ (V = version, e.g. 1.2.3):
...
v=$(python3 tools/version.py)
pio test -e native
pio run -e s3zero
b=.pio/build/s3zero
cp "$b/firmware.bin" "$out/owl-firmware-$v.bin"
tools/sign-firmware.py "$out/owl-firmware-$v.bin" > /dev/null
boot_app0="${PLATFORMIO_CORE_DIR:-$HOME/.platformio}/packages/framework-arduinoespressif32/tools/partitions/boot_app0.bin"
pio pkg exec --package tool-esptoolpy -- esptool.py --chip esp32s3 merge_bin -o "$out/owl-s3zero-merged-$v.bin" \
