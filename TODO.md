...
**Current ticket:** T-40 (`in-progress` — Rollback: mark the new image valid only after BLE is up + 60 s (`verifyRollbackLater`); log/report rollbacks)
...
| ID | Title | Est | Deps | Status | Notes |
|---|---|---|---|---|---|
| T-40 | Rollback: mark the new image valid only after BLE is up + 60 s (`verifyRollbackLater`); log/report rollbacks | 2h | — | in-progress | core has APP_ROLLBACK_ENABLE |
