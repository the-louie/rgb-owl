...
- ~~**Drop the unversioned release copies**~~ — **CLOSED, verified 2026-09-24:** removed from `tools/release.sh` (owl on 1.0.1+, phone app 1.0.2; both match versioned names). Original row kept for the record: **Drop the unversioned release copies** (`owl-firmware.bin(.sig)`, `owl-app.apk` in `tools/release.sh`) once no owl/app runs 1.0.0 (they only know those names)
