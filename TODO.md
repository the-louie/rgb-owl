...
**Current ticket:** T-29 (`in-progress` — BLE WiFi verbs: `wifi_scan`, `wifi_test` (≤15 s, reason), `wifi_save` (only after a passing test), `wifi_forget`; WiFi fields in state)
**Last completed:** T-30 (`done` — commit `dd83621`)
...
| T-29 | BLE WiFi verbs: `wifi_scan`, `wifi_test` (≤15 s, reason), `wifi_save` (only after a passing test), `wifi_forget`; WiFi fields in state | 4h | T-28 | in-progress | |
