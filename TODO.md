...
**Current ticket:** T-42 (`in-progress` — Installer: stream an image URL (HTTPS, CA bundle) into the OTA slot + signature check + progress events; debug-mode `update_url` hook for testing)
**Last completed:** T-41 (`done` — commit `314efe3`)
...
| T-42 | Installer: stream an image URL (HTTPS, CA bundle) into the OTA slot + signature check + progress events; debug-mode `update_url` hook for testing | 4h | T-41 | in-progress | |
