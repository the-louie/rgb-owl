...
**Current ticket:** T-01 (`in-progress` — PlatformIO project: `s3zero` + `native` envs, 4 MB dual-OTA partitions, `main.cpp` stub)
**Last completed:** —
...
|---|---|---|---|---|---|
| T-01 | PlatformIO project: `s3zero` + `native` envs, 4 MB dual-OTA partitions, `main.cpp` stub | 1h | — | in-progress | SPEC §Software |
| T-02 | `layout.h` + XY→index map (constexpr, zig-zag from bottom-right) + native tests | 3h | T-01 | todo | SPEC §LED layout |
